...
def prune_and_evaluate(
    ...
    if pruning_config.recalculate_activations:
        ...
        #TODO: get a flag from prune config here
        if True:
            cripple_out = get_midlayer_activations( opt, pruning_config.cripple,
                pruning_config.collection_sample_size, pruning_config.attn_mode, collect_attn=True )
            #print cripple out object
            #print(f"!!!!cripple_out testing")
            #print(cripple_out.raw["attn"].size())
...
def score_and_prune( opt: Model,
    ...
    if do_attn > 0:
        ...
        if pruning_config.attn_mode == "pre-out":
            #TODO: add offset code here for now -----------------------------------------------------------------
            ##raw_attn_activations = cripple_activations_data.raw["attn"].permute( (1,2,3,0) ).reshape( (opt.cfg.n_layers, opt.cfg.d_model, -1) )
...
def run_pruning(c: PruningConfig):
    ...
    if c.ff_scoring == "random" and c.attn_scoring == "random":
        ...
        raw_attn_activations = cripple_out.raw["attn"].permute( (1,2,3,0) ).reshape( (opt.cfg.n_layers, opt.cfg.d_model, -1) )
        #print(f"size of raw_attn_activations: {raw_attn_activations.size()}")
        #make offset tensor layer x d_model of zeros
        offsets = get_mode_offsets(raw_attn_activations)
        print("Starting to calculate offsets...")
